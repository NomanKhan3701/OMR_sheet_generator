...
const OMRSheet = ({ filters, OMRref }) => {
  ...
  useEffect(() => {
    if (filters.totalQuestion) {
      ...
      for (let i = 1; i <= filters.totalQuestion; i = i + totalQnaInOneColumn) {
        const maxIdx =
          i + totalQnaInOneColumn - 1 > filters.totalQuestion
            ? parseInt(filters.totalQuestion - i)
            : totalQnaInOneColumn;

        newQuestions.push(
          [...Array(totalQnaInOneColumn).keys()].map((item) => {
            if (item > maxIdx) {
              return {
                questionNo: null,
                options: [],
      ...
      <div className={styles.section_container}>
        ...
        <div className={styles.value}>
          {field.value.map((item, idx) => (
            <div className={styles.box} key={idx}></div>
          ))}
        </div>
        <div className={styles.letters_container}>
          {field.value?.map((item, idx) => {
            return (
              <div className={styles.letters} key={idx}>
                {field.characters.map((char, index) => {
                  return (
                    <div
                      className={
                        styles.letter +
                        ` ${values[idx] == char ? styles.active : ""}`
                      }
                      key={`${char}${index}`}
                    >
                      {char}
  ...
  const HorizontalInputSection = ({ field, values = [] }) => {
    return (
      <div className={styles.section_container_horizontal}>
        <div className={styles.title}>{field.title}</div>
  ...
  return (
    <div
      className={styles.OMR_sheet}
      ref={OMRref}
      style={{ "--omr-color": `${filters.color.value}` }}
    >
      <div className={styles.header}>
        ...
        <div className={styles.header_item}>
          <div className={styles.strong_txt}>Omr sheet</div>
        </div>
        {/* <div className={styles.header_item}>
                    <div className={styles.strong_txt}>
                        Barcode
              ...
              <div className={styles.info_container}>
                <div className={styles.instructions_container}>
                  ...
                  <ol className={styles.instructions}>
                    <li>
                      Use only a pencil to fill the circles. Do not use a pen
                    </li>
              ...
              return (
                <div className={styles.section} key={qnaIdx}>
                  {item.map((question, idx) => {
                    const padTop =
                      idx % filters.totalQuestionInOneSection === 0;
                    const padBottom =
                      (idx + 1) % filters.totalQuestionInOneSection === 0 ||
                      (idx + 1) % totalQnaInOneColumn === 0;
                    const borderBottom =
                      padBottom && (idx + 1) % totalQnaInOneColumn !== 0;
                    const extraStyle = padTop
                      ? styles.pad_top
                      : padBottom
                      ? styles.pad_bottom
                      : "";
                    ...
                    return (
                      ...
                      >
                        <div className={styles.question_no + " " + extraStyle}>
                          {question.questionNo}
                        </div>
      ...
      <div className={styles.footer}>
        <div className={styles.signature}>
          <div className={styles.label}>{"Candidate's Signature"}</div>
          <div className={styles.input}></div>
        </div>
        <div className={styles.signature}>
          <div className={styles.label}>{"Invigilator's Signature"}</div>
          <div className={styles.input}></div>
        </div>
      </div>
    </div>
